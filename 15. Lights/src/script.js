...
gui.add(directionalLight.position, 'x').min(0).max(1).step(0.01).name('DirectionalLight position x');
...
gui.add(pointLight, 'distance').min(0.1).max(10).step(0.1).name('PointLight distance');
...
gui.add(spotLight, 'intensity').min(0.1).max(10).step(0.01).name('SpotLight intensity');
gui.add(spotLight, 'penumbra').min(0).max(1).step(0.01).name('SpotLight penumbra');
gui.add(spotLight.target.position, 'x').min(-1).max(1).step(0.01).name('SpotLight Target position x');
scene.add(spotLight);
...
gui.add(material, 'roughness').min(0).max(1).step(0.1).name('Material Roughness');
